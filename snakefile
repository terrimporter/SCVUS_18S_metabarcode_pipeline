""" SCVUS v2 18S metabarcode pipeline
...
Note:
If memory requirements become a problem with the 32-bit USEARCH unoise3 denoising, inidividual runs can be processed up to and including step 5 in their own directories.  To do this remove the hash (#) symbol before 'usearch_out' in rule all, and add a hash to the beginning of rdp_csv2 in rule all (below).  Instead of creating a directory called dir="18Sv4" in the config file at the beginning, call it dir="run1".  Denoised reads from each run (each run1/cat.denoised) should be combined into a single file (ex. 18Sv4/cat.denoised.tmp using the linux cat command).  The config file should then be edited to replace dir="run1" with dir="18Sv4".  Continue by manually dereplicating (vsearch --derep_fulllength) to create 18Sv4/cat.denoised before continuing the pipeline with steps 6 & 7.  
